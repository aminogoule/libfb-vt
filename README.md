...
- ПКМ в любой точке экрана (и над рабочим столом, и поверх окна) открывает
  ...
  «Resolution: WxH» (см. ниже) и, только если ПКМ было по окну, ещё и
  «Close» для этого окна;
...
`fb.h` получил новую точку входа `fb_resize(fb, width, height)` — попытка честного
...
- **`fb.c` (vt_fb/KMS)** и **`fb_vga.c` (legacy VGA)** — всегда возвращают `-1`
  ...
  переключает режимы, закрываются ещё до передачи управления ядру), см.
  обсуждение в истории задач.
