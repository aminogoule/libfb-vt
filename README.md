...
(кроме бэкендов `.vga`/`.svga`, которым `/dev/mem`/`/dev/io` нужны напрямую — см.
[Documentation/backends.md](Documentation/backends.md)).

## Слои

`fb.c`/`fb.h` (framebuffer + двойная буферизация), `ppm.c`/`ppm.h` (загрузчик PPM),
`vtcon.c`/`vtcon.h` (владение VT/консолью), `mouse.c`/`mouse.h` (курсор — sysmouse на
x86/amd64, evdev на arm64), `kbd.c`/`kbd.h` (сырые скан-коды), `proto.c`/`proto.h`
(протокол компоновщика), `fontspleen.h` (встроенный шрифт 8×16), `mgl.c`/`mgl.h`
(программный OpenGL-подобный слой). Подробности —
...
- [Documentation/backends.md](Documentation/backends.md) — четыре бэкенда framebuffer
  (`vt_fb`/`vt_vga`/VMware SVGA II/SVGA3), смена разрешения на лету, переключение VT.
- [Documentation/input.md](Documentation/input.md) — мышь (sysmouse/evdev) и клавиатура:
  xterm mouse-reporting, scrollback, `K_CODE`, диагностика (`MOUSE_DEBUG`, `KBD_DEBUG`,
