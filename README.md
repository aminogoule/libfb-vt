...
- `font8x16.h`        — встроенный моноширинный растровый шрифт 8×16 (IBM VGA ROM, public domain) для отрисовки текста.
...
- `server  [-e команда] [vt]` — **компоновщик (compositor)**: эксклюзивно владеет VT,
  слушает `AF_UNIX`-сокет, выдаёт каждому клиенту разделяемый буфер пикселей на «поверхность»
  (окно), композитит все окна (фон рабочего стола + рамка + заголовок + пиксели клиента) и
  раздаёт ввод с клавиатуры сфокусированному (верхнему) окну. `-e команда` — после захвата VT
  и поднятия сокета запустить `команду` (через `/bin/sh -c`) как первого клиента.
...
Что уже умеет `term`: печать с переносом/прокруткой, C0-управление (CR/LF/BS/TAB/BEL),
CSI-перемещения курсора и очистка (`A B C D H f G d J K`), цвета SGR (`0/1/7/30–37/40–47/90–97/100–107/39/49`),
показ/скрытие курсора (DEC `?25`), заголовки окна (OSC `0`/`2`). Достаточно для интерактивного
shell; это не полный xterm (нет прокручиваемых регионов, UTF-8, мыши).
