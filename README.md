...
`server -k` переключает клавиатуру в `K_CODE` (сырые скан-коды AT/PS2 set 1, `kbd.c`) и
сам отслеживает состояние Shift/Ctrl/Alt, синтезируя правильные xterm-последовательности
с модификатором (`ESC[1;2A` для Shift+Up и т.п.). Обратная сторона: это отдельный, более
низкоуровневый путь ввода — таблица скан-кодов раскладки US QWERTY зашита в `kbd.c` и
**не проверялась на реальном железе этого проекта** (в отличие от F-клавиш и мыши, которые
уже сверены с логами). Прежде чем полагаться на `-k`, проверьте (`KBD_DEBUG` читает именно
`server` — это он декодирует скан-коды, не `term`):
