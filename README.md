...
- `server  image.ppm [vt]`  — зародыш display-сервера: эксклюзивное владение VT + рендер-цикл.
...
Выход по `q` / ESC (или SIGINT/SIGTERM). `server` отказывается переключать VT
(`Alt+Fn`, `Ctrl+Alt+Fn`, `Alt+стрелки`) во время работы; см. раздел ниже.
